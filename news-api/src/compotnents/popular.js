...
function loadPopular() {
    ...
    fetch(api).then((response) => {
        response.json().then((data) => {
            ...
            for(var i = 0; i < 15; i++) {
                popularContainer.innerHTML += "<div> <h2 stle='font-weight = bold'> "+data.articles[i].title+" </h2> <br> "+data.articles[i].description+" <br> <a href = "+data.articles[i].url+"> <img width = 300px; src="+data.articles[i].urlToImage+" </img> </a> </div>";
